**Біо**
---
### 🐾 Коротко про себе:
Художник, початківець у Rust та Red Team. Фановий кібербез-програміст з України.

### 🛠️ Мій сетап:
- **ОС:** Arch (основна), Gentoo
- **WM:** niri + noctalia-shell
- **ПЗ:** Neovim, kitty, fish, zen-browser, aura-emerge
- **Залізо:** ASUS TUF-A15 (Ryzen 7 7435HS + RTX 4060 Max-Q)

### 💻 Мови та скриптинг:
Rust, Bash/Fish, HTML

### 🎯 Поточний фокус:
- Pentesting / Red Team
- Системна розробка на Rust
- Кастомізація Linux-оточення

---
> **Socials:**
>
> 📦 [AUR](https://aur.archlinux.org/packages?K=Deltacat&SeB=m)
>
> ✈️ [Telegram](https://t.me/Deltacat037) • 🎮 [Steam](https://steamcommunity.com/profiles/76561199466585985/) • 🎧 [Discord](https://discordapp.com/users/1024234632000045137)
